...
for $str.chomp.lines -> $line {
    if $line ~~ /^ 'position=<' \s* (\-?\d+) \, \s* (\-?\d+) '> velocity=<' \s* (\-?\d+) \, \s* (\-?\d+) '>' $/ {
        @point.push(Point.new(
            x  => $0.Str.parse-base(10),
            y  => $1.Str.parse-base(10),
            vx => $2.Str.parse-base(10),
            vy => $3.Str.parse-base(10),
        ))
    }
}
...
sub printMessage(Rect $rect, @point) {
    ...
    for @point -> $p {
        @image[$p.y - $rect.miny][$p.x - $rect.minx] = 'x';
    }
    .join(" ").say for @image;
}

sub elapse(@point, Int $second = 1) {
...
class Point {
    ...
    method gist() {
        sprintf "position=<%2s, %2s> velocity=<%2s, %2s>", $!x, $!y, $!vx, $!vy;
    }
}
