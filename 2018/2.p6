...
# 1
{
    my %r = $str.lines.classify({
        given .comb.Bag {
            my \h2 = so .grep(*.value == 2);
            my \h3 = so .grep(*.value == 3);
...
# 2
{
    my @boxes = $str.lines>>.comb();

    for ^+@boxes -> $x {
        for $x ..^ +@boxes -> $y {
            next if $x == $y;
            my @diff = @boxes[$x].cache >>eq<< @boxes[$y].cache;
            if @diff.grep(!*.so).elems == 1 {
                for ^+@diff {
                    @boxes[$x][$_].print if @diff[$_];
                }
            }
        }
    }
