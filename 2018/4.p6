...
class GEvent {
    enum < START ASLEEP WAKEUP DEFAULT >;
}
...
for $str.lines {
    @guardlogs.push: GuardLog.new($_);
}

@guardlogs = @guardlogs.sort({ $^a.cmp($^b) });

for @guardlogs -> $log {
    given $log.event {
        when GEvent::START {
            @orderbydays.push: $log.Guard;
        }
        when GEvent::ASLEEP {
            @orderbydays[* - 1].asleep($log.minute);
        }
        when GEvent::WAKEUP {
            @orderbydays[* - 1].wakeup($log.minute);
        }
        default {
            die "Something wrong!!!";
        }
...
# 1
{
    ...
    my $id = %sleep.sort(*.value).[* - 1].key;

    my @days = @orderbydays.grep(*.id == $id);
...
class GuardLog {
    ...
    my regex log_date   { '[' (\d+) '-' (\d+) '-' (\d+) \s+ (\d+) ':' (\d+) ']' }
    my regex log_start  { 'Guard' \s+ '#'(\d+) \s+ 'begins shift' }
    my regex log_asleep { 'falls asleep' }
    my regex log_wakeup { 'wakes up' }

    sub analyse(Str $str) {
        if $str ~~ /^ <log_date> \s+ <log_start> $/ {
            %{
                date   => Date.new($<log_date>[0].Int, $<log_date>[1].Int, $<log_date>[2].Int),
                hour   => $<log_date>[3].Int,
                minute => $<log_date>[4].Int,
                id     => $<log_start>[0].Int,
                event  => GEvent::START,
            };
        } elsif $str ~~ /^ <log_date> \s+ <log_asleep> $/ {
            %{
                date   => Date.new($<log_date>[0].Int, $<log_date>[1].Int, $<log_date>[2].Int),
                hour   => $<log_date>[3].Int,
                minute => $<log_date>[4].Int,
        ...
        } elsif $str ~~ /^ <log_date> \s+ <log_wakeup> $/ {
            %{
                date   => Date.new($<log_date>[0].Int, $<log_date>[1].Int, $<log_date>[2].Int),
                hour   => $<log_date>[3].Int,
                minute => $<log_date>[4].Int,
    ...
    method new(Str $str) {
        self.bless(:$str, |analyse($str));
    }

    submethod TWEAK() {
        $!event = GEvent::DEFAULT unless $!event.defined;
        $!id = 0 unless $!id.defined;
    }

    method cmp(GuardLog $o) {
        self.date   cmp $o.date ||
        self.hour   <=> $o.hour ||
        self.minute <=> $o.minute;
    }

    method Guard() {
        Guard.new( :$!id, date => $!hour > 0 ?? ($!date.succ) !! $!date );
    }
}
